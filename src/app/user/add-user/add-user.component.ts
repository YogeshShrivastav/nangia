...
export class AddUserComponent implements OnInit {
  ...
  this.datauser = this.db.datauser;
  ...
  getallrole(){
    this.db.FetchData({'access_level':this.datauser.access_level},'User/get_role')
    .subscribe((result)=>{
      console.log(result);
      this.role=result;
  ...
  submituser(f){
    ...
    this.form.news_uploader = this.form.news_uploader || 0;
    ...
    this.submitted = false; 
      ...
      if(result['message'] == "User Added Successfully"){
        ...
        this.router.navigate(['/user-list/0']);
      }else{
