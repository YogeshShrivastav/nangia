...
@Component({
  selector: 'app-recycle-knowledge',
  templateUrl: './recycle-knowledge.component.html'
})
export class RecycleKnowledgeComponent implements OnInit {
  ...
  getallknowlist(pagelimit:any=20,start:any=0,action:any=''){
    ...
    this.db.FetchData({'search':this.search,'start':this.start,'pagelimit':this.pagelimit},'Knowledge/recycle_knowledge_base')
    .subscribe( r=>{
      console.log(r);
  ...
  restore( knowledge_id ) {
    this.db.FetchData({ 'id' : knowledge_id},'Knowledge/restore_knowladge')
    .subscribe( r =>{
      this.getallknowlist();
    }); 
  ...
  delete(knowledge_id,indx)
  {
    this.diolog.delete("Are you sure ")
    .then(resp=>{
      if(resp)
      {
        this.db.FetchData({'access_level':this.db.datauser.access_level,'id':knowledge_id},"Knowledge/delete_knowladge")
        .subscribe(resp=>{
          console.log(resp);
          this.knowcategory.splice(indx,1);
          this.diolog.success("Record deleted successfully !");
        })
      }
      else
      {
        console.log("no");
        this.diolog.warning("Your record is safe !");
      }
    });
    this.getallknowlist();
  }
