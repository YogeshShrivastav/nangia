...
pageSize:any=50;  
  ...
  constructor( public db:DatabaseService, public diolog:DialogComponent,private sanitizer: DomSanitizer) { }
  ...
  getallknowlist(pagelimit:any=20,start:any=0,action:any=''){
    ...
    this.db.FetchData({'search':this.search,'start':this.start,'pagelimit':this.pagelimit},'Knowledge/get_all_Pending_knowledge_base')
      .subscribe( r=>{
      console.log(r);
  ...
  statusChange(id,status, index)
  {
    ...
    this.db.FetchData({ 'id': id },"knowledge/statusChange")
    .subscribe(resp=>{
      // this.knowcategory[ index ].status = status;
    this.getallknowlist(this.pagelimit,   this.start    );
    
  
      this.diolog.success(' Document Approved Successfully! ');
  ...
  changeRejectStatus(id)
  {

    this.db.FetchData({ 'id': id },"knowledge/changeRejectStatus")
    .subscribe(resp=>{

      this.getallknowlist(this.pagelimit,   this.start    );
      this.diolog.error(' Document Rejected!');
...
deleteFunction(knowledge_id,i) {
  ...
  knowladge_delete( knowledge_id,access_level ) {

    console.log(access_level);
    
    this.db.FetchData({ 'id' : knowledge_id,'access_level': access_level },'Knowledge/knowladge_delete') .subscribe( r =>{
